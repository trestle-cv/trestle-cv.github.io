// Deterministic service-contract checks for the Trestle site.
// Run with: node scripts/service-contract-check.mjs

import fs from 'node:fs';
import path from 'node:path';

const here = new URL('.', import.meta.url).pathname;
const service = fs.readFileSync(path.join(here, '..', 'content', 'system-service.html'), 'utf8');
const battle = fs.readFileSync(path.join(here, '..', 'content', 'battle-tested.html'), 'utf8');
const generated = fs.readFileSync(path.join(here, '..', 'public', 'system-service.html'), 'utf8');
...
console.log('trestle service-contract check: ok');