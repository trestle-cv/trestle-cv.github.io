...
must come from a real implementation rather than fictional mockups presented as
working software.
...
6. Commit the source repository second so its gitlink records the exact output
   commit. Both repositories must finish clean.

The site is documentation, not an independent product specification. When site
copy and the application contracts disagree, resolve the product decision in the
application repository first and then update the site.
